...
void DG_Init()
{
	...
	s_Window = XCreateSimpleWindow(s_Display, DefaultRootWindow(s_Display), 0, 0,
				       DOOMGENERIC_RESX, DOOMGENERIC_RESY, 0, blackColor,
				       blackColor);
	...
	s_Image = XCreateImage(s_Display, DefaultVisual(s_Display, s_Screen), depth, ZPixmap, 0,
			       (char *)DG_ScreenBuffer, DOOMGENERIC_RESX, DOOMGENERIC_RESX, 32, 0);
}
...
void DG_DrawFrame()
{
	if (s_Display) {
		...
		XPutImage(s_Display, s_Window, s_Gc, s_Image, 0, 0, 0, 0, DOOMGENERIC_RESX,
			  DOOMGENERIC_RESY);
...
{
	if (s_Window) {
		XChangeProperty(s_Display, s_Window, XA_WM_NAME, XA_STRING, 8, PropModeReplace,

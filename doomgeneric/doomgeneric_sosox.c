...
void DG_Init()
{
	...
	wid = GrNewBufferedWindow(
		...
		"Doom", GR_ROOT_WINDOW_ID, 50, 50, winSizeX, winSizeY, MWRGB(255, 255, 255));

	GrSelectEvents(wid, GR_EVENT_MASK_EXPOSURE | GR_EVENT_MASK_KEY_UP | GR_EVENT_MASK_KEY_DOWN |
				    GR_EVENT_MASK_TIMER | GR_EVENT_MASK_CLOSE_REQ);
...
static unsigned char convert_to_doom_keyx(int wkey)
{
	...
	if ((wkey >= 'a' && wkey <= 'z') || (wkey >= 'A' && wkey <= 'Z') ||
	    (wkey >= '0' && wkey <= '9')) {

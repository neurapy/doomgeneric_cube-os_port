...
static char evdevKeysToASCII1[10] = { '1', '2', '3', '4', '5', '6', '7', '8', '9', '0' };
static char evdevKeysToASCII2[12] = { 'q', 'w', 'e', 'r', 't', 'y', 'u', 'i', 'o', 'p', '[', ']' };
static char evdevKeysToASCII3[12] = { 'a', 's', 'd', 'f', 'g', 'h', 'i', 'j', 'k', 'l', ';', '\'' };
static char evdevKeysToASCII4[11] = { '\\', 'z', 'x', 'c', 'v', 'b', 'n', 'm', ',', '.', '/' };

static char evdevShiftKeysToASCII1[12] = { '!', '@', '#', '$', '%', '^',
					   '&', '*', '(', ')', '{', '}' };
static char evdevShiftKeysToASCII2[12] = { 'q', 'w', 'e', 'r', 't', 'y',
					   'u', 'i', 'o', 'p', '{', '}' };
static char evdevShiftKeysToASCII3[12] = { 'a', 's', 'd', 'f', 'g', 'h',
					   'i', 'j', 'k', 'l', ':', '"' };
static char evdevShiftKeysToASCII4[11] = { '|', 'z', 'x', 'c', 'v', 'b', 'n', 'm', '<', '>', '?' };
...
static void checkKeys()
{
	...
	while (keepGoing) {
		...
		ret	  = poll(pollfds, numInputFds,
				 0); // don't wait at all, just give anything we've got
...
void DG_Init()
{
	...
	fbPtr = mmap(NULL, fbStride * fbHeight, PROT_READ | PROT_WRITE, MAP_SHARED, fbFd, 0);
...
void DG_DrawFrame()
{
	...
	for (int line = 0; line < DOOMGENERIC_RESY; line++) {
		memcpy((void *)((uintptr_t)(fbPtr) + (fbStride * line) + fbOffsetY + fbOffsetX),
		       (void *)(((uintptr_t)DG_ScreenBuffer) +
				(DOOMGENERIC_RESX * line * fbBytesPerPixel)),
